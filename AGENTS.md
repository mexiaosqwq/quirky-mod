...
- **26.2 mixin 的 method 选择器只匹配目标类本类方法**（与 @Shadow/@Invoker 同约束）：@Mixin(LocalPlayer) + method="travel" 启动即崩（"was not located in the target class"）——travel 声明在 Player（LocalPlayer 未覆写）。修复：@Mixin 目标改为声明类（Player）+ handler 内 `(Object) this instanceof LocalPlayer` 过滤（c0c0e41，FlameParticleMixin 同模式）。交付前用 javap/mcsrc 确认方法声明层。
- **自动爬梯（spec §5.9）覆盖面 = 梯子 + 全部藤蔓，排除脚手架**：26.2 `onClimbable()` 判定 = `state.is(#minecraft:climbable)`，该 tag 含脚手架(scaffolding)+全部藤蔓+梯子——脚手架排除（`getInBlockState().is(Blocks.SCAFFOLDING)`），玩家在其上自由走动。单测环境无数据包 tag 数据（`state.is(tag)` 恒 false），tag 分支无法单测，只验证辅助函数（climbVelocity）。
