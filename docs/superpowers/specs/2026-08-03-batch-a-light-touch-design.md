...
- 手持染料对**点燃的**营火右键：消耗 1 个染料，烟柱变为该染料颜色，**限时约 5 分钟（6000 tick）后自动恢复无色**；重染可续期。白染料立即清色。
- 也支持**把染料丢进营火**：染料物品实体接触营火方块时被吞掉并同样染色（贴合原始叙事，两种交互都保留）。
- **白色染料 = 复原**：染白色恢复无色烟，提供不灭火的清除手段。
- **荧光石粉 = 夜光火星（有机扩展，限时）**：丢入荧光石粉，营火在夜晚发出可见的微光火星（远距离信号灯），与颜色叠加生效；**限时约 2 分钟（2400 tick）后自动熄灭**，重投可续期。2026-08-04 玩家反馈“特效太少”，夜光火星加密为每 tick 4 颗向上飘散；后改为限时（原永久不合理）。
...
- 营火熄灭（水浇/铲子扑灭/喷溅水瓶/投射物等任何 LIT 变 false 路径）统一清除颜色与夜光及其倒计时（注入 BlockEntity.setBlockState 检测 LIT true→false），重新点燃后是无色烟。
