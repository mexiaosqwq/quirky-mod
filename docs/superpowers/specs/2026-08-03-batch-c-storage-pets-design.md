...
存箭容器，省背包格子。弓箭手出门带 1-2 个装满的箭袋，弓/弩射击时背包无散装弹药则自动从箭袋抽取（散装优先，箭袋作备用）。

> **设计变更（2026-08-04）**：原 v1 “不做自动抽箭”的决策被推翻——用户明确要求射击时直接用箭袋里的箭。注入点已验单一（`Player.getProjectile` RETURN + `ProjectileWeaponItem.useAmmo` HEAD），创造/多重射击边界可控，风险可接受。
...
- **装入（对齐播种袋，光标点击）**：光标拿弹药左键点箭袋、或光标拿箭袋左键点弹药堆 → 装入（过白名单，受组上限截断，同种已满则余量退回光标，不另开半组）；播放 `BUNDLE_INSERT` 音效。
- **取出（光标点击）**：光标空手右键点箭袋 → 取出一组到光标；或光标拿箭袋右键点格子 → 取出一组到该格（空格放下/同种补充/异种跳过）；播放 `BUNDLE_REMOVE_ONE` 音效。
- **自动抽箭（2026-08-04 新增）**：弓/弩射击时，`Player.getProjectile` 原版找不到散装弹药（返回 EMPTY）→ mixin 遍历背包槽位，找第一个含匹配弹药的箭袋，返回该组副本 + 记录来源（箭袋槽位+组索引）到 tracker；`ProjectileWeaponItem.useAmmo` 检测 projectile 引用==tracker 副本时从箭袋组件扣减（复刻原版 useAmmo 语义，无限附魔不扣、多重射击只扣 1）。多个箭袋按背包槽位顺序，靠前的先用，耗尽后下次 getProjectile 自动切下一个。世界右键无作用（`use`→PASS）。
...
| 事件 | 反馈 |
|---|---|
| 光标装入 | `SoundEvents.BUNDLE_INSERT`（对齐播种袋手感） |
| 光标取出 | `SoundEvents.BUNDLE_REMOVE_ONE` |
| 自动抽箭 | 无额外音效（原版射击音 `ARROW_SHOOT` 已覆盖） |
...
| 场景 | 行为 |
|---|---|
| 箭袋在副手时射击 | 自动抽箭同样查副手槽位（getProjectile 遍历含副手），命中即抽 |
| 两个箭袋都装同种箭 | 背包槽位靠前的先用，耗尽后下次射击自动切下一个 |
| 两个箭袋装不同箭 | 武器弹药谓词决定匹配哪个（弓只匹配 arrows，弩还匹配烟花） |
| 箭袋装箭袋（套娃）| 拒绝：`canFitInsideContainerItems=false`，箭袋不能进收纳袋/潜影盒/另一个箭袋 |
| 创造模式 | 原版 getProjectile 返回 ARROW（不进箭袋分支），创造射击不耗箭袋 |
| 多重射击/穿透 | i==0 那发从箭袋扣 1，i>0 走原版 INTANGIBLE 不扣（与散装一致） |
| 无限附魔 | ammoToUse=0，返回 INTANGIBLE 副本，不从箭袋扣（与散装一致） |
| 把装箭的箭袋丢在地上再捡回 | 内容保留（组件随物品） |
| 箭袋被火烧/仙人掌销毁 | 内容物损失（与潜影盒一致） |
| 空箭袋 tooltip | 显示"空"字样而非空格子图 |
...
| 事件 | 反馈 |
|---|---|
| 吹哨（召集） | **长笛音色（实现采用）**：`SoundEvents.NOTE_BLOCK_FLUTE` 双音上行（音高 1.8/2.0，音量 2.0/1.2）——实测山羊角号角音太像劫掠者号角声、听感沉闷，改用清亮高双音长笛（接近真实口哨）；坐定指挥用更高音高 2.0/2.2 区分。数值可调 |
| 吹哨（坐定指挥） | 哨音短促变体（音高更高 2.0/2.2），与召集哨区分两种用法 |
