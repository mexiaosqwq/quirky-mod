...
- **放置规则**：
  - 对固体方块点击（**任意面**，用户实测反馈原"仅底面"太苛刻）→ 在其下方放一段绳（点击面不限，只要被点方块可支撑；点击地面/实心底座时下方是实心 → isPlaceable 失败自然拒绝）。
...
- 绳 + 灯笼（竖列）→ 挂灯绳段×1（追加获取途径，2026-08-04 用户反馈"找不到道具"后补；原途径为游戏内手持灯笼对绳段右键转换）
